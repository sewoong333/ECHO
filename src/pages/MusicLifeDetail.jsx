...
const dummyComments = {
  1: [
    { id: 1, author: '세션러버', content: '어디서 모임하나요?', time: '30분 전' },
    { id: 2, author: '기타치는사람', content: '장르가 궁금해요!', time: '20분 전' },
    { id: 3, author: '음악인', content: '참여하고 싶어요!', time: '10분 전' },
  ],
  2: [
    { id: 1, author: '드럼짱', content: '연습실 위치가 어디인가요?', time: '1시간 전' },
    { id: 2, author: '락커', content: '장비 제공되나요?', time: '50분 전' },
  ],
  3: [
    { id: 1, author: '공연러', content: '같이 가요!', time: '2시간 전' },
    { id: 2, author: '음악사랑', content: '정보 감사합니다!', time: '1시간 전' },
  ],
};
      ...
      <div style={{ width: '100%', maxWidth: 480, margin: '0 auto', padding: 0, boxSizing: 'border-box' }}>
        <div style={{ width: '100%', background: '#fff', borderRadius: 14, boxShadow: '0 2px 12px rgba(46,216,182,0.08)', padding: 0, overflow: 'hidden', position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'stretch', margin: '32px 0', boxSizing: 'border-box' }}>
          ...
          <div style={{ width: '100%', padding: '32px 20px 24px 56px', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', boxSizing: 'border-box' }}>
            ...
            <div style={{ fontSize: 13, color: '#bbb', marginBottom: 10 }}>댓글 {comments.length}</div>
            <div style={{ width: '100%', marginBottom: 16 }}>
              {comments.length === 0 && <div style={{ color: '#bbb', fontSize: 15 }}>아직 댓글이 없습니다.</div>}
              {comments.map(c => (
                <div key={c.id} style={{ marginBottom: 10, background: '#f8f9fa', borderRadius: 8, padding: '8px 12px', boxShadow: '0 1px 4px #b2f0e6', display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                  <span style={{ fontWeight: 600, color: '#2ed8b6', fontSize: 14 }}>{c.author}</span>
                  <span style={{ color: '#222', fontSize: 15, margin: '2px 0 2px 0' }}>{c.content}</span>
            ...
            <div style={{ width: '100%', display: 'flex', gap: 8 }}>
              <input
                ...
                style={{ flex: 1, padding: '10px 12px', borderRadius: 8, border: '1.5px solid #b2f0e6', fontSize: 15, outline: 'none' }}
                onKeyDown={e => { if (e.key === 'Enter') handleAddComment(); }}
              />
              <button onClick={handleAddComment} style={{ padding: '0 18px', background: '#2ed8b6', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 600, fontSize: 15, cursor: 'pointer' }}>등록</button>
            </div>
          </div>
        </div>
      </div>
