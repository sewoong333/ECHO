...
import { getFirestore, collection, getDocs, updateDoc, doc, query } from 'firebase/firestore';
...
// 게시글에 authorBio 정보 추가
async function addAuthorBio() {
  try {
    console.log('🔄 음악생활 게시글에 authorBio 정보 추가 중...');
    ...
    for (const docSnapshot of musiclifeSnapshot.docs) {
      ...
      // 가상 계정 ID가 있고 authorBio가 없는 경우 업데이트
      if (postData.authorId && virtualAccountInfo[postData.authorId] && !postData.authorBio) {
        const accountInfo = virtualAccountInfo[postData.authorId];
        
        console.log(`🔄 게시글 업데이트: "${postData.title}"`);
        console.log(`   작성자: ${accountInfo.nickname}`);
        console.log(`   Bio 추가: ${accountInfo.bio}`);
    ...
    console.log(`🎉 총 ${updatedCount}개 게시글에 authorBio 정보 추가 완료`);
...
async function main() {
  try {
    console.log('🚀 게시글 authorBio 정보 추가 시작...');
    await addAuthorBio();
